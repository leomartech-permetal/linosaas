...
async function main() {
  const { data: leads } = await supabase.from('leads').select('*').in('whatsapp_number', ['16991415319', '5516991415319', '16991415319@s.whatsapp.net', '5516991415319@s.whatsapp.net', '5516991415319']);
  console.log('Leads:', leads?.map(l => ({ id: l.id, status: l.status, bot_active: l.bot_active, number: l.whatsapp_number })));

  for (const lead of leads || []) {
    const { data: interactions } = await supabase.from('interactions').select('*').eq('lead_id', lead.id);
    console.log(`Interactions for ${lead.whatsapp_number}:`, interactions?.length);
  }
